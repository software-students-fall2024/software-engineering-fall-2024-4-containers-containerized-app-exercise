...
def test_signup_post_fail(client, monkeypatch):
    ...
    """
    monkeypatch.setattr(
        users_collection, "find_one", lambda query: {"username": "existing_user"}
    )
...
def test_login_nonexistent_user(client, monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"username": "nonexistent", "password": "wrong_pass"},
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"Invalid username or password" in response.data
...
def test_dashboard_empty_data(client, monkeypatch):
    ...
    assert (
        "No mood data available yet. Use the camera feed to detect your mood!"
        in response_text
    )
