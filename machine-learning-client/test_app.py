...
def test_perform_sentiment_analysis(sample_sentences_small):
    result = perform_sentiment_analysis(sample_sentences_small)
...
def test_perform_topic_modeling(sample_sentences_large):
    with patch("app.Phrases") as MockPhrases, \
         patch("app.Phraser") as MockPhraser, \
         patch("gensim.corpora.Dictionary") as MockDictionary, \
         patch("gensim.models.LdaModel") as MockLdaModel:
        mock_dict = MagicMock()
        ...
        MockDictionary.return_value = mock_dict
        mock_lda = MagicMock()
        mock_lda.print_topics.return_value = [
            (0, "word1 0.1 word2 0.09"),
            (1, "word3 0.08 word4 0.07")
        ]
        MockLdaModel.return_value = mock_lda
        with patch("app.CoherenceModel", side_effect=Exception("Should not be called")):
            topics = perform_topic_modeling(sample_sentences_large, num_topics=2)
            assert isinstance(topics, list), "Topics should be a list"
            assert len(topics) == 2, f"Expected 2 topics, got {len(topics)}"
            assert all(isinstance(topic, tuple) for topic in topics), "Each topic should be a tuple"

def test_perform_emotion_detection(sample_sentences_small):
    result = perform_emotion_detection(sample_sentences_small)
    assert len(result) == 2, f"Expected 2 results, got {len(result)}"
...
def test_perform_text_summarization(sample_sentences_large):
    summary = perform_text_summarization(sample_sentences_large)
    assert isinstance(summary, str), "Summary should be a string"
    assert len(summary) > 0, "Summary should not be empty"

def test_perform_sentiment_trend_analysis(sample_sentences_small):
    result = perform_sentiment_trend_analysis(sample_sentences_small)
    assert len(result) == 2, f"Expected 2 results, got {len(result)}"
    assert "sentence_index" in result[0], "Missing 'sentence_index' key in the first result"
    assert "compound" in result[0], "Missing 'compound' key in the first result"

def test_perform_overall_emotion_detection():
...
def test_process_document(sample_sentences_large):
    sample_document = { 
        "_id": "1234567890",
        "request_id": "unique_request_id",
        "sentences": sample_sentences_large,
    ...
    with patch("app.perform_sentiment_analysis") as mock_sentiment, patch(
        ...
        mock_sentiment.return_value = sample_sentences_large
        mock_topic_modeling.return_value = ["Topic1", "Topic2"]
        mock_emotion_detection.return_value = sample_sentences_large
        ...
        assert processed_document["overall_status"] == "processed", "Overall status should be 'processed'"
        assert "topics" in processed_document, "Missing 'topics' in the processed document"
        assert "summary" in processed_document, "Missing 'summary' in the processed document"
        assert "sentiment_trend" in processed_document, "Missing 'sentiment_trend' in the processed document"
        assert "overall_emotions" in processed_document, "Missing 'overall_emotions' in the processed document"