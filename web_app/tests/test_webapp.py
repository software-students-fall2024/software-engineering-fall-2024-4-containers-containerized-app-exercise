...
def generate_image_data():
    ...
    return 'data:image/jpeg;base64,' + base64.b64encode(b'test_image_data').decode('utf-8')


@patch('web_app.web_app.users_collection')
@patch('web_app.web_app.requests.post')
def test_signup_get(mock_requests_post, mock_users_collection, test_client):
...
@patch('web_app.web_app.users_collection')
def test_signup_post_missing_fields(mock_users_collection, test_client):
...
@patch('web_app.web_app.users_collection')
def test_login_post_missing_fields(mock_users_collection, test_client):
...
@patch('web_app.web_app.users_collection')
@patch('web_app.web_app.requests.post')
def test_login_post_facial_recognition_success(mock_requests_post, mock_users_collection, test_client):
...
@patch('web_app.web_app.users_collection')
@patch('web_app.web_app.requests.post')
def test_login_post_facial_recognition_failure(mock_requests_post, mock_users_collection, test_client):
    # Mock users_collection.find to return users
    users = [
        {'username': 'user1', 'encoding': [0.1, 0.2, 0.3]},
        {'username': 'user2', 'encoding': [0.4, 0.5, 0.6]},
